...
function prepareureKayit() {
	const faturaDTO = {
		...
		ozelkod: document.getElementById("depo").value || "",
		...
		aciklama: document.getElementById("doviz").value || "",
		dvzcins: document.getElementById("kur").value || "",
		tevoran: document.getElementById("tevoran").value || "",
...
function getTableData() {
	...
	rows.forEach((row) => {
		...
		const rowData = {
			...
			iskonto: parseLocaleNumber(cells[5]?.querySelector('input')?.value),
			miktar: parseLocaleNumber(cells[6]?.querySelector('input')?.value),
			kdv: parseLocaleNumber(cells[8]?.querySelector('input')?.value),
