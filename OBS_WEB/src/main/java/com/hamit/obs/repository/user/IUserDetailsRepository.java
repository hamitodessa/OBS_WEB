...
public interface IUserDetailsRepository extends JpaRepository<User_Details, Long> {
	...
	@Query("SELECT u FROM User_Details u WHERE LOWER(u.user_modul) = LOWER(:userModul) AND LOWER(u.email) = LOWER(:email) AND u.izinlimi = true ORDER BY u.calisanmi DESC")
	List<User_Details> findByUserModulAndEmail(@Param("userModul") String userModul, @Param("email") String email);

	@Query("SELECT u FROM User_Details u WHERE LOWER(u.user_modul) = LOWER(:userModul) AND LOWER(u.email) = LOWER(:email) ORDER BY u.calisanmi DESC")
